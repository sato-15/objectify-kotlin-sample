...
class ClassDiagramGenerator {
    ...
    fun generateUmlFile() {
        ...
        files.forEach {
            generateClassMetaList(it!!)
        }
        writeUmlFile()
    }

    private fun generateClassMetaList(it: Path) {
    ...
    private fun generatePropertyMetaMap(path: Path, classMeta: ClassMeta): MutableMap<String, PropertyMeta> {
    ...
    private fun writeUmlFile() {
        val classDiagramFile = File("class_diagram_${dateFormat.format(Date())}.pu").absoluteFile
